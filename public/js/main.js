...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let currentLevel = 1;
  let timeLeft = 60 * 20; // 20 minutes in seconds
  ...
  problemContainer.innerHTML = `
    <div class="problem-card">
      ...
      <div class="given"><p><strong>Input:</strong> ${randomEquationProblem.given}</p></div>
    </div>
    <div class="problem-card">
  ...
  window.setLevel = function (level) {
    ...
    } else console.error("Invalid level! Choose 1 or 2.");
  };
  ...
  function transformCharacter(char) {
    if (currentLevel === 1) {
      if (/[a-zA-Z]/.test(char)) {
        ...
        return String.fromCharCode(((code - base + ASCII_SHIFT) % 26) + base);
      }
    } else if (currentLevel === 2) {
      if (char === " " || char === "Backspace" || char === "Shift") return char;
      let newCode = char.charCodeAt(0) + ASCII_SHIFT;
      if (newCode > MAX_ASCII) newCode = MIN_ASCII + (newCode - MAX_ASCII - 1);
  ...
  inputField.addEventListener("keypress", function (event) {
    ...
    const transformedChar = transformCharacter(event.key);
  ...
  function updateTimer() {
    ...
    if (timeLeft <= 0) {
      if (!downloadAttempted) disableInputAndDownload();
  ...
  function disableInputAndDownload() {
    ...
    createFile();
    inputField.value = "Wait till time is up for other players!";
  }
  ...
  function handleDownload() {
    if (timeLeft > 0 && !downloadAttempted) {
      const userChoice = confirm(
        "You are downloading before time ends.\nOK: Try again\nCancel: Complete and wait for others."
      );
      if (!userChoice) {
        downloadAttempted = true;
        disableInputAndDownload();
        return;
      } else downloadAttemptedAndRetry = true;
    }
    createFile();
  }
  ...
  // Create file
  function createFile() {
    const transformedText = inputField.value;
    const filename = `${playerName}.${fileTypeSelector.value}`;
    const elapsedTime = 60 * 20 - timeLeft;
    const minutes = Math.floor(elapsedTime / 60);
    const seconds = elapsedTime % 60;
    const timestamp = `Time: ${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}\n---\nOriginal:\n${randomParagraphProblem.description}\n---\nUser:\n`;

    let fileContent = "";
    switch (fileTypeSelector.value) {
      case "txt": fileContent = timestamp + transformedText; break;
      case "cpp": fileContent = `// ${timestamp}\n#include <iostream>\nusing namespace std;\n\n${transformedText}`; break;
      case "java": fileContent = `// ${timestamp}\npublic class Solution {\n    public static void main(String[] args) {\n${transformedText}\n}`; break;
      case "js": fileContent = `// ${timestamp}\n${transformedText}`; break;
      case "py": fileContent = `# ${timestamp}\n${transformedText}`; break;
      default: console.error("Invalid file type!"); return;
    }
